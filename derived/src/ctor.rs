...
use crate::util::{self, FieldListAny};
...
pub fn derive_ctor(input: TokenStream) -> TokenStream {
    ...
    let fields = match util::get_struct_field_list_any(&ast) {
        Ok(f) => f,
        Err(e) => return e,
    };
    ...
    match fields {
        FieldListAny::Named(fields) if fields.is_empty() => quote! {
        ...
        FieldListAny::Named(fields) => {
            ...
            for (fname, ty, attrs) in fields {
                ...
                let is_phantom =
                    ok_else_ret!(util::single_instance_of_attr(attrs, util::ATTR_PHANTOM));
                if !is_phantom {
        ...
        FieldListAny::Unnamed(types) => {
            let mock_ids: Vec<Ident> = types
                .iter()
                .map(|ty| format_ident!("__{}", ty.to_token_stream().to_string()))
                .collect();
            quote! {
                impl #impl_gen #struct_name #ty_gen #where_clause {
                    #func new(#(#mock_ids: #types,)*) -> Self {
                        Self(#(#mock_ids,)*)
                    }
                }
            }
